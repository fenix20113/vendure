...
async function bootstrapWorker(userConfig) {
    ...
    const workerApp = await core_1.NestFactory.createApplicationContext(appModule.AppModule, {
        logger: process.env.VERBOSE_LOGS === '1' ? ['log', 'error', 'warn', 'debug', 'verbose'] : ['error', 'warn'],
    });
    workerApp.useLogger(new vendure_logger_1.Logger());
    workerApp.enableShutdownHooks();
    await validateDbTablesForWorker(workerApp);
    const startJobQueue = () => workerApp.get(job_queue_service_1.JobQueueService).start();

...
function configureGraphQLModule(getOptions) {
    return graphql_1.GraphQLModule.forRootAsync({
        useFactory: (configService, i18nService, idCodecService, typesLoader, customFieldRelationResolverService, cacheManager) => {
            return createGraphQLOptions(i18nService, configService, idCodecService, typesLoader, customFieldRelationResolverService, getOptions(configService), cacheManager);
        },
        inject: [
        ...
        imports: [config_module_1.ConfigModule, i18n_module_1.I18nModule, api_internal_modules_1.ApiSharedModule, service_module_1.ServiceModule.forRoot(), cache_manager_module_1.CacheManagerModule],
    });
}
exports.configureGraphQLModule = configureGraphQLModule;
async function createGraphQLOptions(i18nService, configService, idCodecService, typesLoader, customFieldRelationResolverService, options, cacheManager) {
    var _a;
    const builtSchema = (_a = await readSchemaForApi(options.apiType)) !== null && _a !== void 0 ? _a : await buildSchemaForApi(options.apiType);
    ...
    async function buildSchemaForApi(apiType) {
        ...
        if (Number(process.env.GRAPHQL_SCHEMA_CACHE)) {
            await cacheManager.set(schemaCacheKey(apiType), graphql_2.stripIgnoredCharacters(graphql_2.printSchema(schema)), process.env.GRAPHQL_SCHEMA_CACHE_TTL
     ...
     */
    async function readSchemaForApi(apiType) {
        if (!Number(process.env.GRAPHQL_SCHEMA_CACHE)) {
            return;
        }
        const schema = await cacheManager.get(schemaCacheKey(apiType));
        if (!schema) {
            return;
        }
        return graphql_2.buildSchema(schema);
    }
    function schemaCacheKey(apiType) {
        var _a;
        const prefix = (_a = process.env.GRAPHQL_SCHEMA_CACHE_KEY_PREFIX) !== null && _a !== void 0 ? _a : 'graphql_schema_';

...
let ShopOrderResolver = class ShopOrderResolver {
    constructor(orderService, customerService, sessionService, countryService, activeOrderService) {
    ...
    async orderByCode(ctx, args) {
        if (ctx.authorizedAsOwnerOnly) {
            ...
            if (order) {
                ...
                // time period
                const anonymousAccessLimit = ms_1.default('2h');
                const orderPlaced = order.orderPlacedAt ? +order.orderPlacedAt : 0;
                const activeUserMatches = !!(order &&
                    order.customer &&
                    order.customer.user &&
